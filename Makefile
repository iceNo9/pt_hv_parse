...
ifeq ($(OS),win32)
    ...
    TEST_COMMAND := set PYTHONPATH=$(PYTHONPATH) && poetry run pytest -c pyproject.toml --cov-report=html --cov=pt_hv_parse tests/
else
	PYTHONPATH := `pwd`
    TEST_COMMAND := PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=pt_hv_parse tests/
endif
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 pt_hv_parse tests

check-safety:
	poetry check
	poetry run safety check --full-report
	poetry run bandit -ll --recursive pt_hv_parse tests
